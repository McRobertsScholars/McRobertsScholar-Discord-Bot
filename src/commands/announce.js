...
async function sendDiscordAnnouncement(interaction, announcement) {
  try {
    ...
    await channel.send({
      ...
      allowedMentions: { parse: ["everyone"] }, // Ensure @everyone is allowed
    })

    logger.info(
      `Discord announcement sent by ${interaction.user.tag}${announcement.pingEveryone ? " with @everyone ping" : ""}`,
    )
...
module.exports = {
  ...
  async handleButton(interaction) {
    ...
    if (announcement.userId !== interaction.user.id)
      return interaction.reply({ content: "❌ You can only manage your own announcements!", ephemeral: true })
    ...
    if (action === "preview") {
      ...
      try {
        ...
        await emailService.sendTestEmail(
          config.GMAIL_USER,
          announcement.topic,
          buildAnnouncementText(announcement),
          buildAnnouncementHtml(announcement),
        )
      ...
      const modal = new ModalBuilder()
        .setCustomId(`announce_modal_${subAction}_${announcementId}`)
        .setTitle(`Edit ${subAction === "discord" ? "Discord" : "Email"} Content`)
      const input = new TextInputBuilder()
        .setCustomId("content")
        .setLabel(`${subAction === "discord" ? "Discord" : "Email"} Content`)
        .setStyle(TextInputStyle.Paragraph)
        .setValue(subAction === "discord" ? announcement.discordContent : announcement.emailContent)
        .setMaxLength(4000)
        .setRequired(true)
